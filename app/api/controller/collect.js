...
module.exports = class extends Base {
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const list = yield _this.model('collect').field(['c.*', 'g.name', 'g.list_pic_url', 'g.goods_brief', 'g.retail_price']).alias('c').join({
        ...
        as: 'g',
        on: ['c.value_id', 'g.id']
      }).where({ user_id: _this.getLoginUserId(), type_id: parseInt(typeId) }).countSelect();
  ...
  addordeleteAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const valueId = _this2.post('goodsId');
      const typeId = 0;

      const collect = yield _this2.model('collect').where({ type_id: typeId, value_id: valueId, user_id: _this2.getLoginUserId() }).find();
      let collectRes = null;
      let handleType = 'add';
